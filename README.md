...
| **Crypto Backtest Engine** (`crypto-backtest-engine`) | Event-driven crypto backtesting engine with realistic execution costs, protective exits, position sizing, grid search, walk-forward analysis, and Monte Carlo robustness testing, plus a paper/live trading layer that reuses the same strategy code. | `binance_public_api` optional, `binance_spot_trading` optional, `polymarket_clob_api` optional, `user_input` optional | beta |
